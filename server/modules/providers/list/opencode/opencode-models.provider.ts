...
/**
 * Local-machine default catalog: one free model so a host without the OpenCode
 * CLI (or any session metadata) still resolves a sensible active model.
 */
...
/**
 * Curated OpenCode catalog shipped as immutable defaults.
 *
...
export const OPENCODE_PREDEFINED_MODELS: ProviderModelsDefinition = {
  OPTIONS: [
    ...
    { value: 'opencode/kimi-k2.7-code', label: 'Kimi K2.7', description: 'OpenCode Zen' },
  ...
  DEFAULT: OPENCODE_FALLBACK_MODELS.DEFAULT,
};

/**
 * Lists model ids reported by the installed `opencode` CLI.
 *
 * The plain `opencode models` output is one `<provider>/<model>` id per line
 * which parses cleanly; `--verbose` interleaves JSON metadata with the ids and
 * is reflected instead by the curated {@link OPENCODE_PREDEFINED_MODELS}
 * catalog. Hosts without the CLI return an empty list so callers fall back to
 * the curated catalog and the local default model.
 */
...
/**
 * Merges the curated OpenCode Zen catalog with the ids the local CLI reports,
 * keeping the curated label/description when the same model id appears in both.
 */
export function buildOpenCodeModelsDefinition(): ProviderModelsDefinition {
  ...
  for (const option of OPENCODE_PREDEFINED_MODELS.OPTIONS) {
  ...
  return { OPTIONS: options, DEFAULT: OPENCODE_FALLBACK_MODELS.DEFAULT };
}
...
/** Provider registry model adapter for OpenCode catalog and session metadata. */
