...
test('Codex synchronizer puts the first user message of app-created sessions into summary', { concurrency: false }, async () => {
  ...
  try {
    ...
    await withIsolatedDatabase(async () => {
      ...
      const row = sessionsDb.getSessionById('app-1');
      // The first user message is now the auto-generated summary, not the
      // user-editable custom_name (which stays null until the user renames).
      assert.equal(row?.summary, 'Fix the login redirect bug');
...
test('Codex synchronizer leaves indexed sessions untitled when no name is available', { concurrency: false }, async () => {
  ...
  try {
    ...
    await withIsolatedDatabase(async () => {
      ...
      const row = sessionsDb.getSessionById('codex-indexed-1');
      assert.equal(row?.custom_name, null);
    });
  } finally {
    restoreHomeDir();
